#include <fstream>
#include <iostream>
void get_formated_string(const ptrdiff_t &num1, const ptrdiff_t &num2,
                         const char &op, char resulting_string[],
                         const bool &first) {
  // floating point numbers only need to be printed if '/' is being used
  if (op ^ '/') {
    ptrdiff_t result =
        op == '+' ? num1 + num2
                  : op == '-' ? num1 - num2 : op == '*' ? num1 * num2 : -1;
    sprintf(
        resulting_string,
        "%s(num1 == %td && sign == '%c' && num2 == %td) \n"
...
int main() {
  ...
  output_file << "std::cout << \"Thanks for using this calculator, goodbye "
                 ":)\" << std::endl;\n\n}";
  output_file.close();
